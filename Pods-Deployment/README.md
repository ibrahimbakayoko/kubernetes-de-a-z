# 🚀 Lab2 – Pods & Deployments

Bienvenue dans le **Lab2 – Pods & Deployments** !  
Dans ce deuxième lab, vous allez explorer l’unité fondamentale d’exécution dans Kubernetes : le **Pod**, ainsi que la ressource la plus utilisée pour gérer vos applications : le **Deployment**.

Vous apprendrez comment Kubernetes assure la **scalabilité**, la **tolérance aux pannes** et les **mises à jour continues** grâce aux **ReplicaSets** et aux mécanismes de **rolling updates**.  
À la fin du lab, vous saurez déployer des applications **stateless** de manière propre et professionnelle ⚡

---

## 🧠 Objectifs pédagogiques

- Comprendre le rôle et le fonctionnement des **Pods**
- Découvrir les **Deployments** et les **ReplicaSets**
- Comprendre le **scaling horizontal**
- Introduire les **rolling updates** et les **rollbacks**
- Savoir **nettoyer les ressources** créées lors du lab

---

## ⚙️ Pré-requis

- `kubectl` installé et configuré
- Un cluster Kubernetes local opérationnel (**k3d**, **Minikube** ou **k3s**)
- *(Optionnel)* `Helm` pour les labs suivants

---

## 🧪 Ce que vous allez faire

- Créer et observer un **Pod simple**
- Déployer une application via un **Deployment**
- Visualiser les **ReplicaSets** générés
- Tester le **scaling automatique et manuel**
- Découvrir le mécanisme de **rolling update**
- Nettoyer les ressources en fin de lab

---

## 🚀 Démarrage rapide

```bash
git clone https://github.com/ibrahimbakayoko/kubernetes-de-a-z
cd kubernetes-de-a-z
```

## 📚 Documentation complète
👉 Retrouvez toutes les étapes détaillées, schémas et explications dans le Wiki du projet
📂 Accès complet  
📘 Wiki : [https://lnkd.in/eFYtQCAv](https://github.com/ibrahimbakayoko/kubernetes-de-a-z/wiki/Lab-2-%E2%80%93-Pods-&-Deployments)   
📘 GitHub : [https://lnkd.in/edud6wXX](https://github.com/ibrahimbakayoko/kubernetes-de-a-z/tree/main/Pods-Deployment)

## 🧭 Prochaines étapes
➡️ Lab3 – Services & Networking

➡️ Lab4 – ConfigMaps & Secrets

➡️ Lab5 – Volumes & Persistences

👤 Auteur
Brahima BAKAYOKO 📍 Achères, Île-de-France 💼 Auteur du Lab Kubernetes – Pods & Deployments 📫 Contact : ibra.bakayoko82@gmail.com
